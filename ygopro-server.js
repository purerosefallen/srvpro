...
(function() {
  ...
  init = async function() {
    ...
    if (settings.modules.http.ssl.enabled) {
      if (!neosReuseMainPort) { // 如果没有在上面创建 https 相关对象
        https = require('https');
        httpsOptions = {
          cert: (await fs.promises.readFile(settings.modules.http.ssl.cert)),
          key: (await fs.promises.readFile(settings.modules.http.ssl.key))
    ...
    if (settings.modules.neos.enabled && !neosReuseMainPort) {
      // neos 启用但不复用主端口，单独监听
      ws = require('ws');
      neosHttpServer = null;
      if (settings.modules.http.ssl.enabled) {
