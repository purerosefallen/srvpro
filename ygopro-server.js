...
(function() {
  ...
  ygopro.stoc_follow('GAME_MSG', true, async function(buffer, info, client, server, datas) {
    var card, chain, check, count, cpos, deck_found, found, hint_type, id, j, l, len, len1, len2, len3, limbo_found, line, loc, m, msg_inst, msg_name, n, new_buf, oppo_pos, phase, player, playertype, pos, ppos, record_last_game_msg, ref, ref1, ref2, ref3, ref4, response, room, trigger_location, val, win_pos, win_result;
    room = ROOM_all[client.rid];
    if (!(room && !client.reconnecting)) {
      return;
    }
    ...
    if (settings.modules.heartbeat_detection.enabled && client.pos === 0) {
      if (msg_inst instanceof YGOProMsg.YGOProMsgChaining) {
        card = msg_inst.code;
        found = false;
        for (l = 0, len1 = long_resolve_cards.length; l < len1; l++) {
          id = long_resolve_cards[l];
          if (!(id === card)) {
            continue;
          }
          found = true;
