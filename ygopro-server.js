...
(function() {
  ...
  ROOM_try_persist_replay = global.ROOM_try_persist_replay = function(room) {
    var allow_without_win, buffer, i, j, l, len, len1, player, playerInfos, ref, ref1, replay_filename;
    if (!(room && (settings.modules.mysql.enabled || room.has_ygopro_error))) {
      return false;
    }
    allow_without_win = room.finished || room.has_ygopro_error;
  ...
  ygopro.stoc_follow('GAME_MSG', true, async function(buffer, info, client, server, datas) {
    var act_pos, card, chain, check, count, cpos, deck_found, dialogText, found, hint_type, id, j, l, len, len1, len2, len3, limbo_found, loc, m, msg_inst, msg_name, n, new_buf, oppo_pos, phase, player, playertype, pos, ppos, r_player, record_last_game_msg, ref, ref1, ref2, ref3, response, room, trigger_location, val, victoryWordPlayerList, win_pos, win_result;
    room = ROOM_all[client.rid];
    if (!(room && !client.reconnecting)) {
      return;
    }
    ...
    if (msg_inst instanceof YGOProMsg.YGOProMsgWin) {
      ...
      if (win_result.handled) {
        ...
        if (win_result.recovering) {
          return true;
        }
        pos = win_result.winner;
        if (settings.modules.vip.enabled && room.dueling_players[pos]) {
          victoryWordPlayerList = [room.dueling_players[pos]];
          if (room.hostinfo.mode === 2) {
            victoryWordPlayerList.push(room.dueling_players[pos + 1]);
          }
          for (j = 0, len = victoryWordPlayerList.length; j < len; j++) {
            player = victoryWordPlayerList[j];
  ...
  ygopro.stoc_follow('DUEL_END', true, async function(buffer, info, client, server, datas) {
    var j, l, len, len1, player, ref, ref1, replay_captured, room, state;
    room = ROOM_all[client.rid];
  ...
  ygopro.stoc_follow('REPLAY', true, async function(buffer, info, client, server, datas) {
    var room;
    room = ROOM_all[client.rid];
    if (room && room.duel_finalization.captureReplay(client, buffer)) {
